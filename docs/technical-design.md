...
- `PhoneOnboardingCoordinator` owns Google sign-in handoff, Firebase Phone Auth callback orchestration for protected-number verification, and assistant-name completion because those steps require an Activity surface. Account creation and login are Google-only; phone OTP links the protected mobile number to the already authenticated Firebase user and must never create or switch into a phone-only account.
...
- Android client APIs use Firebase ID tokens from Google sign-in and protected-number phone verification. The backend verifies tokens with Firebase Admin and derives user identity from the verified Firebase `uid`.
...
Google Play onboarding must use production authentication from the first public build. The Android app creates and restores accounts with Firebase Google sign-in, uses Firebase Phone Auth only to link the protected mobile number, and sends Firebase ID tokens to the backend. The backend verifies those tokens with Firebase Admin before creating or reading any user-owned data. Custom bootstrap tokens, in-app development verification codes, and anonymous default-user access are not production paths.
