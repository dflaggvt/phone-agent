...
- Add `GET /v1/onboarding/status` derived from existing backend state. Status: in progress.
- Add Android Setup tab with checklist and action buttons. Status: in progress.
- Add Retell dynamic variables for agent behavior, disclosure, privacy, emergency handling, transfer policy, calendar policy, and outcome expectations. Status: in progress.
- Add tests for onboarding status and agent context injection. Status: in progress.
...
- Add `BillingAccount`, `PaymentMethod`, `SpendingLimit`, `PricePlan`, `BillableMeter`, `CostRateCard`, `UsageEvent`, `RatedUsageEvent`, and `InvoiceMirror` domain models. Status: partial; `BillingAccount`, payment method summary, spending cap, and usage event foundation are implemented.
...
- Add idempotent Personal subscription activation after payment method plus spending cap. Status: in progress.
- Add Stripe Billing Meter event publisher for assistant call minutes and AI processing units. Status: in progress.
- Add rating service for Personal plan, included assistant minutes, overage assistant minutes, and internal AI processing units. Status: partial; customer-facing usage events are recorded, but full rated usage and margin ledger are pending.
- Add internal cost recording for voice minutes, telephony minutes, model tokens, classifier requests, and tool calls. Status: partial; customer-facing call/classification usage exists, detailed provider cost ledger is pending.
...
- Add cap warning and cap reached notification events. Status: partial; cap-reached and inactive-billing blocks now create privacy-safe billing issue notifications, proactive warning thresholds are pending.
- Add Android billing onboarding screen before assistant number assignment. Status: in progress.
- Add billing settings screen with current spend, cap, payment method, invoices, and pause/cancel actions. Status: pending.
...
- Add local invoice mirror and `GET /v1/billing/invoices`. Status: partial; provider-backed invoice listing is implemented, local invoice mirror is pending.
...
- Build production Inbox/Review with review queue and filters. Status: partial; review queue and call rows are implemented, richer filters and communication-item detail parity are pending.
